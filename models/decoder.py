...
from models.resnet import NonBottleneck1D
from models.model_utils import ConvBNAct
...
class DecoderModule(nn.Module):
    ...
    def forward(self, decoder_features, encoder_features):
        ...
        if self.encoder_decoder_fusion == 'add':
            if out.shape[2:] != encoder_features.shape[2:]:
                encoder_features = nn.functional.interpolate(
                    encoder_features, size=out.shape[2:], mode='bilinear', align_corners=False
                )
            out = out + encoder_features
